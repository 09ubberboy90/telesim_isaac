...
from omni.isaac.cortex.dfb import DfContext, DfGoTarget, DfApproachTarget, DfCloseGripper, DfOpenGripper, make_go_home
...
class BuildTowerContext(DfContext):
    ...
    class BlockTower:
        def __init__(self, tower_position, block_height, context):
            ...
            order_preference = ["yellow", "blue", "green", "red"]
    ...
    def mark_block_in_gripper(self):
        ...
        print("Marked block {} as in gripper".format(closest_block.name))
    ...
    def monitor_gripper_has_block(self):
        if self.gripper_has_block:
            ...
            eff_p = self.robot.arm.get_fk_p()
            print("dist to block: %f" % np.linalg.norm(block_p - eff_p))
            if np.linalg.norm(block_p - eff_p) > 0.5:
...
class PickBlockRd(DfStateMachineDecider, DfRldsNode):
        ...
        super().__init__(
            DfStateSequence(
                [
                    ...
                    DfWaitState(wait_time=2.5),
                    DfTimedDeciderState(DfCloseGripper(), activity_duration=5.0),
    ...
    def is_runnable(self):
        ...
        if ct.has_active_block and ct.active_block.has_chosen_grasp:
            ...
            thresh_met = math_util.transforms_are_close(grasp_T, eff_T, p_thresh=0.01, R_thresh=0.005, verbose=False)
            return thresh_met
...
def make_pick_rlds():
    ...
    open_gripper_rd = OpenGripperRd(dist_thresh_for_open=1.5)
    reach_to_block_rd = ReachToBlockRd()
    choose_block = ChooseNextBlock()
    approach_grasp = DfApproachTarget(direction_length=0.25)
...
class ReachToPlaceOnTower(DfDecider):
    def __init__(self):
        super().__init__()
        self.add_child("approach_grasp", DfApproachTarget())
    ...
    def __init__(self):
        ...
        super().__init__(
            DfStateSequence(
                [
                    DfSetLockState(set_locked_to=True, decider=self),
                    DfTimedDeciderState(DfOpenGripper(), activity_duration=5.0),
                    ...
                    # DfTimedDeciderState(DfCloseGripper(), activity_duration=2.5),
                    DfSetLockState(set_locked_to=False, decider=self),
                ]
            )
        )
...
def make_decider_network(robot):
    return DfNetwork(
        BlockPickAndPlaceDispatch(), context=BuildTowerContext(robot, tower_position=np.array([0.35, 0.35, -0.17]))
    )
